...
./build.sh > .release_build_1214.log 2>&1
grep -q 'PRODUCTION_BUILD_PYTHON_INVOCATIONS=0' .release_build_1214.log
grep -q 'GENERAL_PARALLEL_BLIND_RESOURCE_RELEASE=PASS' .release_build_1214.log
grep -q 'WHEELCHAIR_1_2_14_NATIVE_PROFILE_SELECTOR=BUILT' .release_build_1214.log
...
TMP=${TMPDIR:-/tmp}/wheelchair1214_release_$$
mkdir -p "$TMP"
...
# The 1.2.11 regression remains protected: coupled FSI has canonical structural
...
./test_execution_compression_1213.sh
./test_caretaker_1214.sh
echo 'LANGUAGE_PURE_ASSEMBLY=PASS'
echo 'WHEELCHAIR_1_2_14_RELEASE=PASS'
