...
grep -Fq 'PRODUCTION_BUILD_PYTHON_INVOCATIONS=0' .wheelchair_1210_build.log
...
# Reference-only proof of the structural contraction theorem. Production build
# and emitted programs do not execute this Python implementation.
python3 - <<'PY'
import sys
sys.path.insert(0,'surface')
import general_parallel_plan as g
def check(p,edges):
    pred=[[] for _ in range(p['node_count'])]; succ=[[] for _ in range(p['node_count'])]
    for u,v in edges: succ[u].append(v); pred[v].append(u)
    for region in p['causal_region_indices']:
        for u,v in zip(region,region[1:]):
            assert v in succ[u] and len(succ[u])==1 and len(pred[v])==1
chain=[(i,i+1) for i in range(31)]
p=g.causal_geometry([f'c{i}' for i in range(32)],chain,4)
assert p['causal_region_count']==1 and p['materialized_node_contexts']==1 and p['fused_node_count']==31
check(p,chain)
diamond=[(0,1),(0,2),(1,3),(2,3)]
p=g.causal_geometry(['a','b','c','d'],diamond,4)
assert p['causal_region_count']==4 and p['fused_node_count']==0; check(p,diamond)
mixed=[(0,1),(0,2),(1,3),(2,4),(3,5),(4,6)]
p=g.causal_geometry([str(i) for i in range(7)],mixed,4)
assert p['causal_region_count']==3 and p['fusion_rule']=='producer_outdegree_one_and_consumer_indegree_one'
assert p['fusion_runtime_selector'] is False and p['fusion_workload_identity'] is False
assert p['runtime_fixed_home_ownership']==0 and p['post_completion_work_search']==0
assert p['resource_release_destination']==0 and p['resource_consumer_visibility']=='none'; check(p,mixed)
...
for name in ('branch','iterate'):
  for q in (2,4):
    ...
    assert n['cpu_width']==q and 0<n['node_context_materialization']<n['native_fragment_count']
    ...
    assert n['runtime_selector'] is False and n['global_ready_queue']==0 and n['root_scheduler']==0
    assert n['work_stealing']==0 and n['serial_fallback']==0 and n['runtime_fixed_home_ownership']==0
    assert n['post_completion_work_search']==0 and n['resource_release_destination']==0 and n['resource_handoff']==0
    b=n['blind_release_causal']; assert b['resource_consumer_visibility']=='none'
    assert b['release_rule']=='owned_to_free_no_recipient'
print('GENERAL_PARALLEL_NATIVE_CAUSAL_REGION_1210=PASS')
PY

# Different executor widths are allowed different physical ELF layouts.  The
# invariant is equal semantics/numerics, never byte identity across q1/q2/q4.
...
for q in 1 2 4; do
  ...
  build/rankn_1210/r6_q$q 4 > "build/rankn_1210/r6_q$q.out"
  [ "$(cat build/rankn_1210/r6_q$q.out)" = 'checksum_bits=0x40bfc00000000000' ]
  readelf -d "build/rankn_1210/r6_q$q" 2>&1 | grep -Fq 'There is no dynamic section'
done
cmp build/rankn_1210/r6_q1.out build/rankn_1210/r6_q2.out
cmp build/rankn_1210/r6_q1.out build/rankn_1210/r6_q4.out
[ ! -e build/topologyc-rankn ]
echo 'RANK_N_CURRENT_DERIVED_PROFILE_1210=PASS'
echo 'RANK_N_EXECUTOR_SEMANTIC_EQUIVALENCE_1210=PASS'
