#!/usr/bin/env python3
"""Derive generic base/expanded native register-pressure profiles.

The transformations are structural compiler capabilities. They are not tied to a
workload, benchmark, physics domain, or named optimization recipe.
"""
from pathlib import Path

SRC = Path('build/tensor_frontend_product_subtract_residency.S')
BASE = Path('build/tensor_frontend_profile_base.S')
WIDE = Path('build/tensor_frontend_profile_wide.S')
...
# calculation temporarily installs canonical IEEE binary64 MXCSR and restores it.
anchor = '''.rpf_no:\n    xor eax,eax\n    mov edx,1\n    ret\n\n# gen_load_f64_bits'''
...
# Expanded generic register profile. Runtime-owned ZMM12..15 remain untouched.
# Persistent dependency/CSE ownership expands only through free ZMM16..29;
# ZMM30..31 remain immutable constant carriers.
wide = text
repls = [
...
for a,b in repls:
    if a not in wide:
        raise SystemExit(f'expanded register profile anchor missing: {a[:60]}')
...
print('NATIVE_RESOURCE_PROFILE_BASE=DERIVED')
print('NATIVE_RESOURCE_PROFILE_WIDE=DERIVED')
print('NATIVE_RESOURCE_PROFILE_RUNTIME_RESERVED_ZMM12_15=PROTECTED')
print('NATIVE_RESOURCE_PROFILE_AOT_MXCSR_CANONICAL=PASS')
print('NATIVE_RESOURCE_PROFILE_SUBNORMAL_RECIPROCAL_REJECT=PASS')
print('NATIVE_RESOURCE_PROFILE_SCALAR_FALLBACK=0')
